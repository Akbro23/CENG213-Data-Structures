...
std::vector<KeyValuePair<K, V> > TreeMap<K, V>::subMap(K fromKey, K toKey) const {
    std::vector<KeyValuePair<K, V> > vector;
    if (stree.getMin().getKey() > toKey || stree.getMax().getKey() < fromKey)
        return vector;
    KeyValuePair<K, V> startPair = stree.getCeiling(fromKey);
    KeyValuePair<K, V> endPair = stree.getFloor(toKey);
    while (startPair != endPair){
        vector.push_back(startPair);
        startPair = stree.getNext(startPair.getKey());
    }
    vector.push_back(endPair);
    return vector;
}
